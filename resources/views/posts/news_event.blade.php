        ...
        <div class="col-lg-4">
            <div class="card hover-shadow-lg hover-translate-y-n10">
              <a href="/posts/{{$post->slug}}">
                <img alt="Image placeholder" src="/storage/{{$post->image}}" class="card-img-top">
              </a>
              <div class="card-body py-5 text-center">
                <a href="/posts/{{$post->slug}}" class="d-block h5 lh-150">{{$post->title}}</a>

                <p>Posted by: 
                <a href="/{{$post->user->username}}">
                {{$post->user->username}}</p></a>
                <h6 class="text-muted mt-4 mb-0">{{date('M j, Y h:ia',strtotime($post->created_at))}}</h6>
              </div>
